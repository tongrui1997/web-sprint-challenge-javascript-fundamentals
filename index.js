...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: Because internal is a closure, the inner function has access to the variable of the outer function
...
function lowerCaseNames(arr) {
  /*Your Code Here*/
  const result = arr.map(item => `${item["animal_name"].toLowerCase()}`);
  return result;
}
...
function lowPopulationAnimals(arr) {
  /*Your Code Here*/
  const result = arr.filter(item => item.population < 5);
  return result;
}
...
function USApop(arr) {
  /*Your Code Here*/
  const pop = arr.reduce(function(accumulator, item) {return accumulator + item["population"]}, 0);
  return pop;
}
